    ...
    <div className="space-y-4">
      {canManage && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {group && <span className="text-[11px] text-gray-400">Nowe pliki trafią do materiałów grupy „{group.name}" i będą widoczne dla jej członków.</span>}
        </div>
      )}
      ...
      {linked.length === 0 ? (
        <div className="p-6 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl text-center">
          ...
          {canManage && <p className="text-xs text-gray-400 mt-1">Dodaj plik lub podepnij istniejące materiały grup domowych.</p>}
        </div>
      ) : (
        <div className="space-y-2">
          {linked.map((m) => (
            <div key={m.id} className="flex items-center justify-between p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl">
              <div className="flex items-center gap-3 min-w-0">
                <div className="bg-accent-secondary-lighter dark:bg-accent-secondary-darkest/40 p-2 rounded-lg text-accent-secondary dark:text-accent-secondary-light shrink-0"><FileText size={18} /></div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <a href={fileUrl(m.storage_path)} target="_blank" rel="noreferrer" title="Otwórz" className="p-2 rounded-lg text-accent-secondary hover:bg-accent-secondary-lightest"><LinkIcon size={16} /></a>
                {canManage && <button onClick={() => unlink(m)} title="Odepnij od wydarzenia" className="p-2 rounded-lg text-gray-400 hover:text-red-500"><Trash2 size={16} /></button>}
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="p-3 overflow-y-auto custom-scrollbar">
              {pickList === null ? <Spinner center size={22} /> : filteredPick.length === 0 ? (
                ...
                <div key={f.id} className="flex items-center justify-between gap-2 p-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <button onClick={() => attach(f)} className="text-xs px-2.5 py-1.5 rounded-lg bg-accent-primary text-white whitespace-nowrap">Podepnij</button>
                </div>
              ))}
            </div>
          </div>
